...
def route_login(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to contact cloud login API")
    except KeyError:
        raise HTTPException(status_code=500, detail="Invalid parameters provided")

    if resp.ok:
        auth_resp: Dict[str, str] = resp.json()
        UserAuth.set_auth({"user": auth["username"], "token": auth_resp["token"]})
        return auth_resp
    else:
        try:
            error_data = resp.json()
        except ValueError:
            error_data = {"error": "Unknown error from cloud login API"}
...
def sync_account():
    ...
    if len(UserAuth.get_auth()["token"]) != 0:
        try:
            resp = requests.post(f"http://{settings.cloud_hostname}:{settings.cloud_port}/api/user/validate", json=UserAuth.get_auth())
        except requests.RequestException:
            print(f"Error contacting cloud login API")
            raise HTTPException(status_code=500, detail="Failed to contact cloud login API")
        except KeyError:
